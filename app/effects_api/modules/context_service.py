...
async def _get_context_boundaries(project_id: int) -> gpd.GeoDataFrame:
    ...
    geometries = [await urban_api_gateway.get_territory_geometry(territory_id) for territory_id in context_ids]
    return gpd.GeoDataFrame(geometry=geometries, crs=4326)


async def _get_context_roads(project_id: int):
    gdf = await urban_api_gateway.get_physical_objects_scenario(project_id, physical_object_function_id=26)
    return gdf[['geometry']].reset_index(drop=True)


async def _get_context_water(project_id: int):
    gdf = await urban_api_gateway.get_physical_objects_scenario(project_id, physical_object_function_id=4)
    return gdf[['geometry']].reset_index(drop=True)
...
async def get_context_blocks(project_id: int):
    project_boundaries = await urban_api_gateway.get_project_geometry(project_id)
...
async def get_context_functional_zones(project_id: int, token: str = None) -> gpd.GeoDataFrame:
    ...
    functional_zones = await urban_api_gateway.get_functional_zones_scenario(project_id, year, source)
    return adapt_functional_zones(functional_zones)


async def get_context_buildings(project_id: int):
    gdf = await urban_api_gateway.get_physical_objects_scenario(project_id, physical_object_type_id=1, centers_only=True)
...
async def get_context_services(project_id: int, service_types: pd.DataFrame):
    gdf = await urban_api_gateway.get_services_scenario(project_id, centers_only=True)
