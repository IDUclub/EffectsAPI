...
from app.effects_api.constants.const import (
    ...
    ROADS_ID,
    WATER_ID,
    LAND_USE_RULES, SOCIAL_INDICATORS_MAPPING,
)
...
from app.effects_api.modules.service_type_service import adapt_service_types, adapt_social_service_types_df
...
class ContextService:
    ...
    async def _get_project_boundaries(self, project_id: int, token: str) -> gpd.GeoDataFrame:
    ...
    async def _get_context_boundaries(self, project_id: int, token: str) -> gpd.GeoDataFrame:
        ...
        geometries = [await self.client.get_territory_geometry(tid) for tid in context_ids]
        return gpd.GeoDataFrame(geometry=geometries, crs=4326)

    async def _get_context_roads(self, scenario_id: int, token: str) -> gpd.GeoDataFrame:
    ...
    async def _get_context_water(self, scenario_id: int, token: str) -> gpd.GeoDataFrame:
    ...
    async def get_context_functional_zones(
        ...
        functional_zones = await self.client.get_functional_zones(scenario_id, year, source, token)
    ...
    async def get_context_buildings(self, scenario_id: int, token: str) -> gpd.GeoDataFrame | None:
        """
    ...
    async def get_context_territories(self, project_id: int, token: str) -> gpd.GeoDataFrame:
        """
        ...
        context_ids = project["properties"]["context"]
        data = [
            {"parent": territory_id, "geometry": await self.client.get_territory_geometry(territory_id)}
            for territory_id in context_ids
        ]
        return gpd.GeoDataFrame(data=data, crs=4326)

    async def load_context_blocks(self, scenario_id: int, token: str) -> Tuple[gpd.GeoDataFrame, int]:
        """
    ...
    async def assign_land_use_context(
        ...
        """
        fzones = await self.get_context_functional_zones(scenario_id, source, year, token)
    ...
    async def enrich_with_context_services(
        ...
        if not sdict:
            logger.info(f"No context services to aggregate for scenario_id={scenario_id}")
            return blocks

        for stype, services in sdict.items():
            services = services.to_crs(blocks.crs)
            b_srv, _ = aggregate_objects(blocks, services)
            b_srv[["capacity", "count"]] = b_srv[["capacity", "count"]].fillna(0).astype(int)
    ...
    async def aggregate_blocks_layer_context(
        ...
        blocks = await self.assign_land_use_context(blocks, scenario_id, source, year, token)
    ...
    async def get_shared_context(
        ...
        ctx_blocks_path = self.cache.save_gdf_artifact(
            ctx_blocks, method=method, owner_id=project_id, params=params, name="context_blocks", fmt="pkl"
        )
        ...
        service_types_path = self.cache.save_df_artifact(
            service_types, method=method, owner_id=project_id, params=params, name="service_types", fmt="parquet"
        )

        self.cache.save(
            method, project_id, params,
            {
