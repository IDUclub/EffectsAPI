...
async def _get_context_roads(project_id: int):
    gdf = await urban_api_gateway.get_physical_objects(project_id, physical_object_function_id=26)
    return gdf[['geometry']].reset_index(drop=True)


async def _get_context_water(project_id: int):
    gdf = await urban_api_gateway.get_physical_objects(project_id, physical_object_function_id=4)
    return gdf[['geometry']].reset_index(drop=True)
...
async def get_context_functional_zones(project_id: int, source: str, year: int, token: str) -> gpd.GeoDataFrame:
    ...
    functional_zones = await urban_api_gateway.get_functional_zones(project_id, year, source)
    return adapt_functional_zones(functional_zones)


async def get_context_buildings(project_id: int):
    gdf = await urban_api_gateway.get_physical_objects(project_id, physical_object_type_id=4, centers_only=True)
