...
class EffectsService:
    ...
    async def run_values_oriented_requirements(
        ...
        if not (
            cached_curr
            and cached_curr["meta"].get("scenario_updated_at") == updated_at_curr
        ):
        ...
        soc_values_map = await self.urban_api_client.get_social_values_info()
        table_current = _extract_values_table(
            cached_curr["data"]["result"], soc_values_map
        )
        ...
        if base_id:
            ...
            if not base_updated_at:
                info_base = await self.urban_api_client.get_scenario_info(
                    base_id, token
                )
            ...
            if (
                cached_base
                and cached_base["meta"].get("scenario_updated_at") == base_updated_at
                and "result" in cached_base["data"]
            ):
                table_base = _extract_values_table(
                    cached_base["data"]["result"], soc_values_map
                )
            else:
                ...
                if (
                    cached_base
                    and cached_base["meta"].get("scenario_updated_at")
                    == base_updated_at
                    and "result" in cached_base["data"]
                ):
                    table_base = _extract_values_table(
